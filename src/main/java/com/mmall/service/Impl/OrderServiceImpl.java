...
public class OrderServiceImpl implements IOrderService{
    ...
    public ServerResponse pay(Long orderNo,Integer userId,String path){
        ...
        // 如果该字段为空，则默认为与支付宝签约的商户的PID，也就是appid对应的PID,我们已经配置在zfbinfo.properties里了
        String sellerId = "";
        ...
        //整合orderItemList到支付宝的规范goodsDetailList
...
/*        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                File folder = new File(path);//这个路径就是tomcat/webapps/ROOT/upload
                if (!folder.exists()){
                ...
                //由支付宝提供的方法，自动生成二维码图片，并存储到qrPath
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId){
        ...
        List<Cart> cartList = cartMapper.selectCheckedByUserId(userId);//查两次表，IO次数很大，考虑外连接改进
        ...
        for (OrderItem orderItem : orderItemList){
            orderItem.setOrderNo(order.getOrderNo());//由于生成的订单号仅仅在order里面，orderNo是OrderNo和order唯一的联系
        }
        //更新orderItem后，须插入数据库，使用mybatis批量插入
    ...
    public ServerResponse<PageInfo> manageSearch(Long orderNo,int pageNum,int pageSize){
        ...
        if (order!=null){
            ...
            //这里骚操作，因为查找是根据orderNo找的，所以List里面只有一个order
    ...
    public ServerResponse<String> manageSendGoods(Long orderNo){
        ...
        if (order!=null){
            if (order.getStatus() == Const.OrderStatusEnum.PAID.getCode()){
                order.setStatus(Const.OrderStatusEnum.SHIPPING.getCode());//更新为已发货
